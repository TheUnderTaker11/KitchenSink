...
public class CustomLevelToolFixingx9 implements IRecipe{
		// 012
		// 345
		// 678
	@Override
	public boolean matches(InventoryCrafting inv, World worldIn) 
	{
		...
		for(int i=0; i < inv.getSizeInventory(); ++i)
		{
			...
			if (item != null && item.getItem() == Itemsss.LevelPick && item.getTagCompound() != null) 
			{
				...
				if(tag.getInteger("dur")<=(tag.getInteger("maxdur")-900))
				{
					++pick;
				}
			}
			if(item != null && inv.getStackInSlot(i).getItem()==inv.getStackInSlot(i).getItem().getItemFromBlock(Blocks.iron_block))
			{
				++ironcount;
			}
