	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker)
    {
		if(attacker instanceof EntityPlayer)
		{
			...
			if(!(target instanceof EntityPlayer)&&stack.getTagCompound()!=null&&stack.getTagCompound().getString("ownerID").equals(player.getUniqueID().toString()))
			{
				...
				if(stack.getTagCompound().getBoolean("active")) target.playSound(SoundEvents.BLOCK_ANVIL_LAND, 3.0F, 0.1F);
			...
			else target.attackEntityFrom(DamageSource.causePlayerDamage(player), 50);
